...
def admin_page() -> None:
    ...
    with tab1:
        ...
        if submitted and value:
            try:
                ...
                st.error(f"Could not add value: {exc}")
    ...
    with tab2:
        ...
        if submitted and name and email and password:
            run_sql(
                "INSERT INTO users (name, email, password_hash, role, active) VALUES (:name, :email, :password_hash, :role, :active)",
                {"name": name, "email": email.lower(), "password_hash": hash_password(password), "role": role, "active": True},
            )
            st.success("User added.")
            st.rerun()

        with st.form("reset_password", clear_on_submit=True):
            active_users = read_df("SELECT email FROM users WHERE active = TRUE ORDER BY email")
        ...
        if reset_submitted and reset_email and new_password:
            run_sql(
